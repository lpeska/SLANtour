...
class CentralniDataEnt
{
    ...
    function __construct($id, $nazev, $text)
    {
        ...
        $this->text = str_replace('[$id_objednavka]', "<i>bude doplněno po dokončení objednávky</i>", $this->text);
    }
    ...
    public function loadPaymentViewData()
    {
        switch ($this->id) {
            case self::ID_PLATBA_HOTOVE:
                $this->nazevWeb = 'Hotově';
                $this->class = 'method-h';
                break;
            case self::ID_PLATBA_PREVODEM:
                $this->nazevWeb = 'Bankovním převodem';
                $this->class = 'method-bp';
                break;
            case self::ID_PLATBA_SLOVENSKO:
                $this->nazevWeb = 'Bankovním převodem v eurech na Slovensku';
                $this->class = 'method-bp';
                break;
