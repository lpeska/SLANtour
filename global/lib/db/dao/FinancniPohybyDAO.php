...
class FinancniPohybyDAO
{
    ...
    private static function readSelectedZajezdListWithObjednavky($filter, $idSerial)
    {
        ...
        while (@$row = $stmt->fetch(PDO::FETCH_OBJ)) {
            ...
            while (@$rowObj = $stmtObj->fetch(PDO::FETCH_OBJ)) {
                ...
                $slevy = self::readSlevyByObjednavkaId($rowObj->id_objednavka);
    ...
    private static function readSluzbyByObjednavkaAndZajezdId($idObjednavka, $idZajezd)
    {
        ...
        $stmt = self::$db->query(FinancniPohybySQLBuilder::readSluzbyByObjednavkaAndZajezdIdSQL(array($idObjednavka, $idZajezd, $idObjednavka)));
        while (@$row = $stmt->fetch(PDO::FETCH_OBJ)) {
    ...
    private static function readSlevyByObjednavkaId($idObjednavka)
    {
        ...
        $stmt = self::$db->query(FinancniPohybySQLBuilder::readSlevyByObjednavkaIdSQL(array($idObjednavka)));
        while (@$row = $stmt->fetch(PDO::FETCH_OBJ)) {
            if($row->castka_slevy > 0) {//pokud je spocitana sleva, zustanu pouze u ni
