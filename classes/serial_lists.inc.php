...
class Serial_list extends Generic_list {
    ...
    private $description = array(
        ...
        "poznavaci-zajezdy" => "",
        "lazenske-pobyty" => " - Česko, Morava, Slovensko, Maďarsko, Německo - lázně, wellness, termály, ubytování, pobyty pro seniory...",
        "za-sportem" => " - Premier League, Serie A, Formule 1, Moto GP, Tenisové turnaje, LOH 2016 Rio, zápasy Českých národních týmů",
        ...
        "pobyty-hory" => " - Ubytování a pobyty v Krkonoších, Jeseníkách, Beskydech, na Šumavě, Malé Fatře, Vysokých či Nízkých Tatrách...",
...
function order_by_subquery($vstup) {
  ...
  //      echo $this->threshold_orders." - ".$this->radek["suma_objednavek"];
    ...
    function show_filtr($typ_zobrazeni = "") {
        ...
        //tvroba input datum rezervace od
        $input_cena_od = "Cena od: <input id=\"cena_od\" name=\"cena_od\" type=\"text\" style=\"width:80px;\" value=\"" . ($_SESSION["cena_od"]==0?(""):($_SESSION["cena_od"])) . "\"  /> Kč       		
                        ";
        //tvroba input datum rezervace do
        $input_cena_do = " do: <input id=\"cena_do\" name=\"cena_do\" type=\"text\" style=\"width:80px;\" value=\"" . ($_SESSION["cena_do"]==0?(""):($_SESSION["cena_do"])) . "\"  /> Kč       		
                        ";
        
        $order_by = " Seřadit: <select name=\"order_by\">
                         <option value=\"recommended\" ".($this->order_by=="recommended"?( " selected=\"selected\""):("")).">Doporučené zájezdy</option>
                        <option value=\"cena_up\" ".($this->order_by=="cena_up"?( " selected=\"selected\""):("")).">Od nejlevnějšího</option>
                        ...
                        <!--<option value=\"most_wanted\" ".($this->order_by=="most_wanted"?( " selected=\"selected\""):("")).">Nejžádanější</option>-->
                        </select>";
        ...
        if ($typ_zobrazeni == "pod_nadpisem") {
            ...
            $vystup = "
                    <div>
			<form method=\"post\" name=\"terminy\" action=\"?update_filter=1" . $promenne . "\">
                            Země: " . $input_zeme . " " . $text . "<br/>" . $input_datum_od . $input_datum_do . $submit . "
			</form>
                    </div>
                    ";
    ...
    function show_list_item($typ_zobrazeni, $text_max = "", $text_min = "", $text_ideal = "", $show_ajax = false) {
        ...
        if ($typ_zobrazeni == "termin_list_index") {
            ...
            $vypis = "<tr $suda><td>" . $vyprodano . "<a " . $color . " href=\"/zajezdy/zobrazit/" . $this->get_nazev_ubytovani_web() . "/" . $this->get_id_zajezd() . "\">
                                ...
                                <td>" . $this->change_date_en_cz($this->get_termin_od()) . " - " . $this->change_date_en_cz($this->get_termin_do()) . ($this->get_dlouhodobe_zajezdy() ? ("<span title=\"Odjezd a příjezd libovolně v rámci zadaného termínu\" style=\"color:green;font-weight:bold;font-size:1.2em;\">*</span>") : ("")) . "
            ...
            $vypis = "<tr $suda><td>" . $vyprodano . "<a " . $color . " href=\"/zajezdy/zobrazit/" . $this->get_nazev_ubytovani_web() . "/" . $this->get_id_zajezd() . "\">
                                " . $this->change_date_en_cz($this->get_termin_od()) . " - " . $this->change_date_en_cz($this->get_termin_do()) . "</a>" . ($this->get_dlouhodobe_zajezdy() ? ("<span title=\"Odjezd a příjezd libovolně v rámci zadaného termínu\" style=\"color:green;font-weight:bold;font-size:1.2em;\">*</span>") : ("")) . "
                              ...
                              <tr " . $class . " style=\"border-bottom:1px solid black;\">
                                ...
                                <td >    
                                    <a  href=\"/zajezdy/zobrazit/" . $this->get_nazev_web() . "/" . $this->get_id_zajezd() . "\" >
                                     ...
                                     " . $this->change_date_en_cz($this->get_termin_od()) . " - " . $this->change_date_en_cz($this->get_termin_do()) . ($this->get_dlouhodobe_zajezdy() ? ("<span title=\"Odjezd a příjezd libovolně v rámci zadaného termínu\" style=\"color:green;font-weight:bold;font-size:1.2em;\">*</span>") : ("")) . "                                
            ...
            while ($row = mysqli_fetch_array($data)) {
                ...
                if ($k == 4) {
                    ...
                    if($show_cena_u_terminu){
                        $act_sleva_text = ", <span style=\"color:green;\">" . $cena . " Kč</span><br/>";
                    }else{
                        $act_sleva_text = "";
                    }
                    
                    $terminy .= "&nbsp;&nbsp;<a href=\"/zajezdy/zobrazit/" . $this->get_nazev_web() . "/" . $row["id_zajezd"] . "\"   class=\"object_link\" >" . $this->change_date_en_cz($row["od"]) . " - " . $this->change_date_en_cz($row["do"]) . ($this->get_dlouhodobe_zajezdy() ? ("<span title=\"Odjezd a příjezd libovolně v rámci zadaného termínu\" style=\"color:green;font-weight:bold;font-size:1.2em;\">*</span>") : ("")) . "</a>".$act_sleva_text ;
                }
            }
            ...
            //nechci aby se nedokončené zájezdy zobrazovaly
            ...
            $res = array(
                ...
                "foto_url" => "https://slantour.cz/foto/nahled/".$this->get_foto_url(),
                "lokace" => $lokace,
            ...
            while ($row = mysqli_fetch_array($data)) {
                ...
                if ($k == 3) {
                    ...
                    $terminy .= "&nbsp;&nbsp;<a href=\"/zajezdy/zobrazit/" . $row["nazev_web"] . "/" . $row["id_zajezd"] . "\"  class=\"object_link\">" . $this->change_date_en_cz($row["od"]) . " - " . $this->change_date_en_cz($row["do"]) . ($this->get_dlouhodobe_zajezdy() ? ("<span title=\"Odjezd a příjezd libovolně v rámci zadaného termínu\" style=\"color:green;font-weight:bold;font-size:1.2em;\">*</span>") : ("")) . "</a>" .
                            (($show_cena_u_terminu == 1) ? (", <span style=\"color:green;\">" . $row["akcni_cena"] . " Kč</span><br/>") : ("") );
                }
            }
            ...
            $vypis = "
                              ...
                              <tr " . $class . " style=\"border-bottom:1px solid black;\">
                                ...
                                <td >    
                                    ...
                                    <span style=\"color:red;font-weight:bold;\">cena od: " . $this->get_castka() . " Kč</span>
            ...
            $vypis = "
                              ...
                              <tr " . $class . " style=\"border-bottom:1px solid black;\">
                                ...
                                <td >    
                                   ...
                                   <b style=\"color:green;\">cena od: " . $this->get_castka() . " Kč</b>
                    ...
                    if ($last_serial != $row["nazev"]) {                       
                        ...
                        if ($this->typ_pozadavku == "select_ubytovani_group" and $max_sleva_serial > 0) {
                          if($max_sleva_serial_mena=="%"){
                             ...
                             $last_sleva_serial = "<b >SLEVA AŽ " . $max_sleva_serial . "Kč</b> ";                                                   
                          }
                            $max_sleva_serial=0;
                            $max_sleva_serial_prepocitana_procenta_castka=0;
...
//                echo "</pre>";
                ...
                if ($row["akcni_cena"] > 0 and $row["cena_pred_akci"] > 0) {
                    ...
                    if ($act_sleva > $max_sleva_procenta) {
                        ...
                        $text_slevy .= $act_sleva."% akční sleva ".$row["popis_akce"]."\n";
                    }
                    $sleva_z_akce = 1;
                }
                ...
                foreach ($sleva_array_mena_zajezd as $key => $mena) {
                    if($sleva_array_castka_zajezd[$key]>0){
                        ...
                        if($mena=="Kč" and $max_sleva_kc < $sleva_array_castka_zajezd[$key]){
                ...
                foreach ($sleva_array_mena as $key => $mena) {
                    if($sleva_array_castka[$key]>0){
                        ...
                        if($mena=="Kč" and $max_sleva_kc < $sleva_array_castka[$key]){
                    ...
                    if($max_sleva_procenta_castka > $max_sleva_kc){
                        ...
                        $act_max_sleva_mena="%";
                            if($max_sleva_procenta > 3 and $sleva_z_akce == 0){
                                ...
                                $sleva_zajezd_text = "<span style=\"color:red;text-decoration:line-through;font-size:0.8em;\">".$row["castka"]." Kč</span> <b style=\"color:green;\" title=\"".$text_slevy."\">po slevě: <span itemprop=\"price\">". $castka_po_sleve . "</span><meta itemprop=\"priceCurrency\" content=\"CZK\" /> Kč</b> ";
                            }else if($sleva_z_akce == 1){                                  
                                $sleva_zajezd_text = "<span style=\"color:red;text-decoration:line-through;font-size:0.8em;\">".$row["cena_pred_akci"]." Kč</span> <b style=\"color:green;\" title=\"".$text_slevy."\">po slevě: <span itemprop=\"price\">". $row["akcni_cena"] . "</span><meta itemprop=\"priceCurrency\" content=\"CZK\" /> Kč</b> ";
                            }else{
                                $sleva_zajezd_text = "<b style=\"color:green\" itemprop=\"price\">".$row["castka"]." Kč</b><meta itemprop=\"priceCurrency\" content=\"CZK\" />";
                            }
...
//                        $showall_text.="<![if !IE]><a href=\"" . $_SERVER["request_uri"] . "?all=" . $this->get_id_serial() . "\"  style=\"color:#505090;\"><i>zobrazit všechny termíny</i></a><![endif]>";
                ...
                if($k <= $max_k or $showall or $show_extra_zajezd){
                    ...
                    if ($row["nazev_zajezdu"]) {
                        if (preg_match("/[0-9]+\. *[0-9]+/", $row["nazev_zajezdu"]) != 0) {
                            ...
                            $podnazev .= "<a href=\"/zajezdy/zobrazit/" . $row["nazev_web"] . "/" . $row["id_zajezd"] . "\" class=\"object_link\">" . $row["nazev_zajezdu"] . ", " . $datum . "</a>" . ($row["dlouhodobe_zajezdy"] ? ("<span title=\"Odjezd a příjezd libovolně v rámci zadaného termínu\" style=\"color:green;font-weight:bold;font-size:1.2em;\">*</span>") : (""));
                        }
                    } else {
                        $podnazev .= "<a href=\"/zajezdy/zobrazit/" . $row["nazev_web"] . "/" . $row["id_zajezd"] . "\" class=\"object_link\">" . $datum . "</a>" . ($row["dlouhodobe_zajezdy"] ? ("<span title=\"Odjezd a příjezd libovolně v rámci zadaného termínu\" style=\"color:green;font-weight:bold;font-size:1.2em;\">*</span>") : (""));
                    }

                    
                    if($row["castka"] <= "1"){//podezřelá cena - rozhodně nechceme zobrazovat cenu za korunu
                        $text_cena = "<b style=\"color:green\"> Předběžná registrace</b>";
                    }else{
            ...
            if($max_sleva > 0 ){
               if($max_sleva_mena=="%"){
                   ...
                   if($max_sleva<=500){
                       ...
                       $vypis_sleva = "<div class=\"sleva\">SLEVA " . $sleva_text ."<span class=\"sleva_serial$sleva_style\">". $max_sleva . "<span>Kč</span></span></div>";
            ...
            $vypis = "                            
                              <tr " . $class . " >
                                ...
                                <td style=\"font-family:arial;\">    
                                    ...
                                    cena od: <span style=\"color:red;font-weight:bold;\">" . $this->get_castka() . " Kč</span>                           
                          ";
            return $vypis;
            ...
            $vypis.= "
                                ...
                                <span style=\"color:red;font-weight:bold;\">od: " . $this->get_castka() . " Kč</span>
                           <br/>
                            ";
            ...
            $vypis.= " <tr><td  style=\"padding-left:10px;\">                              
                                ...
                                <span style=\"color:red;font-weight:bold;\">od: " . $this->get_castka() . " Kč " . $sleva . "</span>
           ...
           if(strlen($this->get_nazev())>=25){
            ...
            $vypis = "<tr $suda><td>" . $vyprodano . "<a " . $color . " href=\"/zajezdy/zobrazit/" . $this->get_nazev_ubytovani_web() . "\">
                                ...
                                <td style=\"text-align:right;\"><span style=\"font-weight:bold;color:#009a00;font-size:1.2em;\">" . $this->get_castka() . " Kč</span>
                            </tr>
                            ";
            return $vypis;
	    ...
	    $contextMenu = " oncontextmenu=\"objectOpenedFromRecomendedListParams(".$this->get_id_serial().",'".$objParam."');\"";
            ...
            $vypis = "<div class=\"objekt\" id=\"".$this->get_id_serial()."\" style=\"background-color:white;text-align:center;\"><a " . $color . " href=\"/zajezdy/zobrazit/" . $this->get_nazev_web() . "\" class=\"object_link\" $click>
                                ...
                                <span style=\"font-weight:bold;color:#009a00;font-size:1.2em;\">cena od: " . $this->get_castka() . " Kč</span>
                                 </div>   
                            ";
	    /*
	    if($this->test == 0){
	...
	*/
    ...
    function show_strankovani() {
        ...
        $vypis = "<div class=\"strankovani\"><a href=\"?str=0" . $destinace . "\" title=\"první stránka zájezdů\">&lt;&lt;</a> &nbsp;";
        ...
        $vypis = $vypis . " &nbsp; <a href=\"?str=" . $posl_str . $destinace . "\" title=\"poslední stránka zájezdů\">&gt;&gt;</a></div>";
