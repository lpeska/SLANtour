  ...
  function show_map(){
	...
	function show_keyword(){
        ...
        function get_cena_pred_akci() {
            return
                "døíve: <span style=\"color:red;text-decoration:line-through;font-size:1.2em;\">".
                $this->serial["cena_pred_akci"]." Kè</span>";

        }
        function get_akcni_cena() {
            return " <br/> nyní od: <span style=\"color:#00ae35;text-decoration:none;font-size:1.2em;font-weight:bold;\">".
                $this->serial["akcni_cena"]." Kè</span>";
		...
		while($sleva = mysqli_fetch_array($data)){
                        if($sleva["sleva_staly_klient"] == 1){
                            $text_slevy = "<span style=\"color:grey;font-size:0.8em;font-style:italic\">Sleva bude odeètena po zkontrolování Vašich údajù pracovníkem CK</span>";
                        }else{
                            $text_slevy = "<span style=\"color:grey;font-size:0.8em;font-style:italic\">Sleva bude odeètena v prùbìhu objednávky</span>";                        
                        }                    
	...
	function get_popis_stravovani()  { 
		if($this->serial["popis_stravovani"]!=""){
			return "<h3 class=\"plain_text\"><b>STRAVOVÁNÍ</b></h3>
	...
	function get_program_zajezdu()  { 
		if($this->serial["program_zajezdu"]!=""){
			return "<h3 class=\"plain_text\"><b>PROGRAM ZÁJEZDU</b></h3>\n".$this->serial["program_zajezdu"];
		}else{ 
			return "";
			}
	}
	...
	function get_popis() { 
		if($this->serial["popis"]!=""){
			return "<h3 class=\"plain_text\"><b>PODROBNÝ POPIS</b></h3>".$this->serial["popis"]."";
		}
	}
	
	function get_cena_zahrnuje() { 
		if($this->serial["cena_zahrnuje"]!=""){
			return "<h3 class=\"plain_text\"><b>ZÁKLADNÍ CENA ZAHRNUJE</b></h3>".$this->serial["cena_zahrnuje"]."";
		}
	}
	
	function get_cena_nezahrnuje() { 
		if($this->serial["cena_nezahrnuje"]!=""){
			return "<h3 class=\"plain_text\"><b>ZÁKLADNÍ CENA NEZAHRNUJE</b></h3>".$this->serial["cena_nezahrnuje"]."";
		}
	}
	
	function get_poznamky() { 
		if($this->serial["poznamky"]!="" or $this->serial["poznamka_ubytovani"]!=""){
                        $ret = "<h3 class=\"plain_text\"><b>POZNÁMKY</b></h3>";
		...
		while($sleva = mysqli_fetch_array($data_zajezd)){
			...
			$vystup .= "<tr><td>
                                    <span style=\"font-size:2.2em;color:#009015;font-weight:bold;float:left;font-style:italic\" title=\"Sleva až ".$sleva_procenta."%\"/><span class=\"serial_sleva\">".$sleva_procenta."<span>%</span></span>
                                        ...
                                        <strong style=\"font-size:1.2em;\">AKCE:</strong> cena pøed akcí: <span style=\"color:red;text-decoration:line-through;font-size:1.2em;\">".$this->serial["cena_pred_akci"]."</span> Kè<br/>
                                        nyní cena od: <span style=\"color:green;font-size:1.6em;font-style:italic;\"><b>".$this->serial["akcni_cena"]." Kè</b></span></td></tr>";
                }
		$vystup .= "</table>";	
