...
* 	- je koncipovana jako singleton (práva jsou potøeba i ve vìtšinì podtøíd)
*/
...
class Agentura extends Generic_data_class{
	...
	private function __construct($name, $passwd, $from_sessions=0, $hlavni_administrator=0){
		...
		if($this->jmeno==""){
			...
			$this->chyba("Uživatel nepøihlášen");
			$this->uzivatel=array();
		}else{
					...
					if(!$this->get_error_message() ){//nactu seznam prav
						...
						if($from_sessions==0){
							...
							if(!$this->get_error_message()){
								$this->confirm("Uživatel byl úspìšnì pøihlášen");
							}						
						}
					}
				}else{
...
/*vytvoreni dotazu na zaklade typu pozadavku*/
	...
	function show_info_about_user($typ_zobrazeni=""){
		if($typ_zobrazeni==""){
			$vystup="
				<div class=\"uzivatel\">
				<p>Uživatel: <strong>".$this->get_uzivatelske_jmeno()." (".$this->get_nazev().")</strong>
				<span style=\"margin-left:60%;\">/<a href=\"?typ=logout\" title=\"odhlásí aktuálnì pøihlášeného uživatele\">odhlásit</a>/</span></p>
				</div>";
			return $vystup;
	...
	function show_main_menu(){
		...
		$vystup = $vystup."[<a href=\"/xml_creator.php\">Generování XML</a>] \n</div>";
		return $vystup;				
	}
		...
		if($typ_zobrazeni=="tabulka"){
			...
			$vystup="<table class=\"list\">
							<tr>
								...
								<th>vytvoøit objekt</th>
								<th>upravit svùj objekt</th>
								<th>upravit cizí objekt</th>
								<th>smazat svùj objekt</th>
