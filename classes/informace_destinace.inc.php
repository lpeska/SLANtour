...
/**na zaklade textoveho vstupu vytvori korektni cast retezce pro order by*/
	...
	function get_popis_short() { 
			...
			if(500-$delka_popisek > 0){
				$delka_popis_short = strrpos( substr( strip_tags($this->radek["popis"])  ,0,700-$delka_popisek)  ," ")  ;
	...
	function get_popisek() { 
		if($this->radek["popisek"]!=""){
			...
			return nl2br(substr(strip_tags($this->radek["popis"]),0,strrpos(substr(strip_tags($this->radek["popis"]),0,350)," ")))."...";
		}
	}			
