...
class Foto_zapas extends Generic_list{
...
//------------------- METODY TRIDY -----------------	
	...
	function create_query($typ_pozadavku){
		if($typ_pozadavku=="show"){
			$dotaz="select `foto_zapas`.`id_zapas`,
							`foto`.`id_foto`,`foto_zapas`.`zakladni_foto`,
							`foto`.`nazev_foto`,`foto`.`popisek_foto`, `foto`.`foto_url` 
					  from `foto_zapas` join
							`foto` on (`foto`.`id_foto` =`foto_zapas`.`id_foto`)
					where `foto_zapas`.`id_zapas`= ".$this->id_serialu."
					order by `foto_zapas`.`zakladni_foto` desc,`foto`.`id_foto` ";
			//echo $dotaz;
			return $dotaz;
			
		}else if($typ_pozadavku=="remove_zakladni_foto"){
			$dotaz= "UPDATE `foto_zapas`
								SET `zakladni_foto`=0
								WHERE `id_zapas`=".$this->id_serialu." and `zakladni_foto`=1";
			//echo $dotaz;
			return $dotaz;		
			
		}else if($typ_pozadavku=="create"){
			$dotaz=$dotaz.
						"INSERT INTO `foto_zapas`
							(`id_zapas`,`id_foto`,`zakladni_foto`)
						VALUES
			...
			$dotaz=$dotaz. "UPDATE `foto_zapas`
						SET `zakladni_foto`=".$this->zakladni_foto."
						WHERE `id_zapas`=".$this->id_serialu." and `id_foto`=".$this->id_foto."
						LIMIT 1;";
			//echo $dotaz;
			return $dotaz;		
			
		}else if($typ_pozadavku=="delete"){
			$dotaz= "DELETE FROM `foto_zapas`
						WHERE `id_zapas`=".$this->id_serialu." and `id_foto`=".$this->id_foto."
						LIMIT 1";
			//echo $dotaz;
			return $dotaz;		
	...
	function show_list_item($typ_zobrazeni){
		if($typ_zobrazeni=="tabulka"){
			...
			if(!$this->get_zakladni_foto()){
				$zakl_foto="<a href=\"?id_zapas=".$this->get_id_serial()."&amp;id_foto=".$this->get_id_foto()."&amp;typ=foto&amp;pozadavek=update&amp;zakladni_foto=1\">zmìnit na základní foto</a> | ";
			}else{
				$zakl_foto="<a href=\"?id_zapas=".$this->get_id_serial()."&amp;id_foto=".$this->get_id_foto()."&amp;typ=foto&amp;pozadavek=update&amp;zakladni_foto=0\">odznaèit základní</a> | ";
			}
			...
			$vypis=$vypis."
							...
							<td class=\"menu\">
								".$zakl_foto."
							  <a href=\"?id_zapas=".$this->get_id_serial()."&amp;id_foto=".$this->get_id_foto()."&amp;typ=foto&amp;pozadavek=delete\">delete</a>
	...
	function get_id_serial() { return $this->radek["id_zapas"];}
