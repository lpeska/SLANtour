...
class Foto_aktuality extends Generic_list{
...
//------------------- METODY TRIDY -----------------	
	...
	function create_query($typ_pozadavku){
		if($typ_pozadavku=="show"){
			$dotaz="select `foto_aktuality`.`id_aktuality`, 
							`foto`.`id_foto`,	`foto`.`nazev_foto`,`foto`.`popisek_foto`, `foto`.`foto_url` 
					  from `foto_aktuality` join
							`foto` on (`foto`.`id_foto` =`foto_aktuality`.`id_foto`) 
					where `foto_aktuality`.`id_aktuality`= ".$this->id_aktuality." 
			...
			$dotaz=$dotaz.
						"INSERT INTO `foto_aktuality`
							(`id_aktuality`,`id_foto`)
						VALUES
			...
			$dotaz= "DELETE FROM `foto_aktuality` 
						WHERE `id_aktuality`=".$this->id_aktuality." and `id_foto`=".$this->id_foto."
						LIMIT 1";
			//echo $dotaz;
			return $dotaz;		
	...
	function show_list_item($typ_zobrazeni){
		...
		if($typ_zobrazeni=="tabulka"){
			...
			$vypis=$vypis."
							...
							<td class=\"menu\">
							  <a href=\"".$adresa_modulu."?id_aktuality=".$this->id_aktuality."&amp;id_foto=".$this->get_id_foto()."&amp;typ=foto&amp;pozadavek=delete\">odebrat</a>
