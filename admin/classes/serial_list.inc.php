    ...
    function create_query($typ_pozadavku, $only_count = 0)
    {
        ...
        if ($typ_pozadavku == "show" || $typ_pozadavku == "selector") {
                ...
                if($prehled_obsazenosti or $pokrocily_filtr){
                    if($_SESSION["f_zobrazit_zajezdy"]==1){
                        ...
                        $from_zajezdy = "group_concat(`zajezd`.`id_zajezd` order by `zajezd`.`id_zajezd` separator \";\") as `id_zajezd`,
                                        ...
                                        group_concat(`zajezd`.`nezobrazovat_zajezd` order by `zajezd`.`id_zajezd` separator \";\") as `nezobrazovat_zajezd`,
                    ...
                    if($_SESSION["f_zobrazit_zajezdy"]==1){
                        $select_zajezdy = "group_concat(DISTINCT`zajezd`.`id_zajezd` order by `zajezd`.`id_zajezd` separator \";\") as `id_zajezd`,
                                        ...
                                        group_concat(DISTINCT `zajezd`.`id_zajezd`,'_',`zajezd`.`nezobrazovat_zajezd` order by `zajezd`.`id_zajezd` separator \";\") as `nezobrazovat_zajezd`,
                                        group_concat(DISTINCT `zajezd`.`id_zajezd`,'_',`zajezd`.`od` order by `zajezd`.`id_zajezd` separator \";\") as `od`,
                                        group_concat(DISTINCT `zajezd`.`id_zajezd`,'_',`zajezd`.`do` order by `zajezd`.`id_zajezd` separator \";\") as `do`,
           ...
           if($prehled_obsazenosti == 1){
                ...
                $select = "select SQL_CALC_FOUND_ROWS `serial`.`id_serial`,`serial`.`nazev`,`serial`.`nazev_web`,`serial`.`id_sablony_zobrazeni`,`serial`.`nezobrazovat`,
                                ...
                                group_concat(DISTINCT `objekt`.`typ_objektu` order by `objekt`.`nazev_objektu` separator \";\") as `typ_objektu`,
                                `typ_serial`.`nazev_typ`";
            }else{
    ...
    function show_filtr()
    {
       ...
       $zajezdy_pz = "<td style=padding:5px;><input type=\"hidden\" name=\"prehled_zajezdu\" id=\"prehled_zajezdu_switch\" value=\"".($this->show_prehled_zajezdu_filtr>0?"1":"-1")."\">
        ...
        $objednavky = "<td class='vybery-zso' >1+ objednávka <input type='checkbox' name='f_zajezd_objednavka' value='1' onchange=\"filtr_change_zobrazit_zajezdy();\" ".($_SESSION["f_zajezd_objednavka"]==1?(" checked=\"checked\""):(""))." />
            ...
            <td class='vybery-NZO'>NEZOBRAZOVAT ";
...
//chybi id_typ a id_podtyp
        ...
        if (!$this->get_error_message()) {
            ...
            if ($typ_zobrazeni == "tabulka" or $typ_zobrazeni == "tabulka_slevy" or $typ_zobrazeni == "tabulka_ts_prehled_objednavek") {
                ...
                if($this->show_pokrocily_filtr){
                    ...
                    if($this->radek["nezobrazovat"]==1){
                        $vypis = "<tr class=\"vybery-NZO\">";           
                    }else if($this->radek["pocet_zajezdu"]==0){
                ...
                if ($this->radek["nazev_ubytovani"] != "") {                    
                    ...
                    $array_typ_objektu = explode(";", $this->radek["typ_objektu"]);
                    
                    foreach ($array_objekty as $key => $value) {
                        if($array_typ_objektu[$key]==5) {
                              $ubyt .= "<a href=\"/admin/objekty.php?id_objektu=" . $array_id_objektu[$key] . "&typ=tok_list&pozadavek=show_letuska\">" . $value . "</a><br/>";
                        }  else{
                ...
                if ($this->moznosti_editace == "zadne") {
                    ...
                    $vypis = $vypis . " <form style=\"font-size:0.9em;\" action=\"" . $adresa_modulu . "?id_serial=" . $this->radek["id_serial"] . "&amp;typ=serial&amp;pozadavek=copy\" method=\"post\"><strong>kopírovat seriál</strong>: nový název <input type=\"text\" name=\"nazev\" style=\"width:300px;\" /> Kopírovat posl. zájezd: <input type=\"checkbox\" name=\"copy_zajezd\" value=\"1\" title=\"Kopírovat také poslední zájezd\"/> <input type=\"submit\" value=\"&gt;&gt;\" /></form>";
                ...
                if($this->zobrazit_zajezdy){
                    ...
                    $nezobrazovat_zajezdy = explode(";", $this->radek["nezobrazovat_zajezd"] );
                    ...
                    if($zajezd_ids[0]>0){
                        ...
                        foreach ($zajezd_ids as $key => $id) {
                            $od = $this->change_date_en_cz(str_replace($id."_","",$od_array[$key]));
                            $do = $this->change_date_en_cz(str_replace($id."_","",$do_array[$key]));
                            $nezobrazovat_zajezd = str_replace($id."_","",$nezobrazovat_zajezdy[$key]);
                            ...
                            if($nezobrazovat_zajezd == 1){
                                $vypis .= "<tr class=\"vybery-NZO\">";
                            }else if(is_array($pocty_osob) and array_sum($pocty_osob)>0){
    ...
    private function computePocetOsob($pocty_osob, $typ_zobrazeni=""){
        ...
        $sum_osob = $pocty_osob[3]+$pocty_osob[4]+$pocty_osob[5]+$pocty_osob[6]+$pocty_osob[8]+$pocty_osob[10];
         if($typ_zobrazeni == "array") {
             $radek_pocet_osob = array((intval($sum_osob)-intval($pocty_osob[8])-intval($pocty_osob[10])),intval($pocty_osob[3]),intval($pocty_osob[4]),intval($pocty_osob[5]),intval($pocty_osob[6]),intval($pocty_osob[8]),intval($pocty_osob[10]));
             return $radek_pocet_osob;
         }                  
                            
                            $radek_pocet_osob = "<strong>".(intval($sum_osob)-intval($pocty_osob[8])-intval($pocty_osob[10]))."</strong>
    ...
    private function computePocetObjednavek($pocty_objednavek, $typ_zobrazeni=""){
                            ...
                            $sum_objednavek = $pocty_objednavek[3]+$pocty_objednavek[4]+$pocty_objednavek[5]+$pocty_objednavek[6]+$pocty_objednavek[8]+$pocty_objednavek[10];
        if($typ_zobrazeni == "array") {
             $radek_objednavka = array((intval($sum_objednavek)-intval($pocty_objednavek[8])-intval($pocty_objednavek[10])),intval($pocty_objednavek[3]),intval($pocty_objednavek[4]),intval($pocty_objednavek[5]),intval($pocty_objednavek[6]),intval($pocty_objednavek[8]),intval($pocty_objednavek[10]));
             return $radek_objednavka;
         } 
                            $radek_objednavka = "<strong>".(intval($sum_objednavek)-intval($pocty_objednavek[8])-intval($pocty_objednavek[10]))."</strong>
