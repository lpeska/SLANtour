...
class Zapas_list extends Generic_list{
	//vstupni data
	...
	//kontrola vstupnich dat	
		...
		if( $this->legal() ){		
			...
			if(mysqli_num_rows($this->data)==0){
				$this->chyba("Pro zadané podmínky neexistuje žádný zápas!");
			}
...
//------------------- METODY TRIDY -----------------	
	...
	function create_query($typ_pozadavku,$only_count=0){
		...
		if($typ_pozadavku=="show"){
			...
			if($only_count==1){
				$select="select count(`zapas`.`id_zapas`) as pocet";
				$limit="";
			}else{
				$select="select `zapas`.`id_zapas`,`zapas`.`nazev`,`zapas`.`datum`";
			}
...
/**na zaklade textoveho vstupu vytvori korektni cast retezce pro order by*/
	function order_by($vstup){
		switch ($vstup) {
			case "id_up":
				 return "`zapas`.`id_zapas`";
   			 break;
			case "id_down":
				 return "`zapas`.`id_zapas` desc";
   			 break;			 				 			 
			case "nazev_up":
				 return "`zapas`.`nazev`";
   			 break;
			case "nazev_down":
				 return "`zapas`.`nazev` desc";
   			 break;
                        case "datum_up":
				 return "`zapas`.`datum`";
   			 break;
			case "datum_down":
				 return "`zapas`.`datum` desc";
   			 break;
		}
			...
			$vystup="
				<table class=\"list\">
					<tr>
						<th>id
						<div class='sort'>
							<a class='sort-up' href=\"?typ=zapas_list&amp;pozadavek=change_filter&amp;pole=ord_by&amp;ord_by=id_up".$vars."\"></a>
							<a class='sort-down' href=\"?typ=zapas_list&amp;pozadavek=change_filter&amp;pole=ord_by&amp;ord_by=id_down".$vars."\"></a>
							</div>
						</th>
						<th>Název
						<div class='sort'>
							<a class='sort-up' href=\"?typ=zapas_list&amp;pozadavek=change_filter&amp;pole=ord_by&amp;ord_by=nazev_up".$vars."\"></a>
							<a class='sort-down' href=\"?typ=zapas_list&amp;pozadavek=change_filter&amp;pole=ord_by&amp;ord_by=nazev_down".$vars."\"></a>
							</div>
						</th>
                                                <th>Datum
                                                <div class='sort'>
							<a class='sort-up' href=\"?typ=zapas_list&amp;pozadavek=change_filter&amp;pole=ord_by&amp;ord_by=datum_up".$vars."\"></a>
							<a class='sort-down' href=\"?typ=zapas_list&amp;pozadavek=change_filter&amp;pole=ord_by&amp;ord_by=datum_down".$vars."\"></a>
							</div>
						</th>
						<th>Možnosti editace
						</th>
	...
	function show_list_item($typ_zobrazeni){
		...
		if($typ_zobrazeni=="tabulka"){
			...
			$vypis = $vypis."
							<td class=\"id\">".$this->radek["id_zapas"]."</td>
							<td class=\"nazev\">".$this->radek["nazev"]."</td>
                                                        <td class=\"nazev\">".$this->change_date_en_cz($this->radek["datum"])."</td>
							<td class=\"menu\">";			 
			if($this->moznosti_editace=="select_serial_objednavky"){
				$vypis = $vypis."
								<a href=\"".$adresa_modulu."?id_zapas=".$this->radek["id_zapas"]."&amp;typ=zajezd_list&amp;moznosti_editace=select_zajezd_objednavky\">vybrat zapas</a>";
			}else{					 
				$vypis = $vypis."
								<a href=\"".$adresa_modulu."?id_zapas=".$this->radek["id_zapas"]."&amp;typ=zapas&amp;pozadavek=edit\">zápas</a>";
					
				$vypis = $vypis." | <a href=\"".$adresa_modulu."?id_zapas=".$this->radek["id_zapas"]."&amp;typ=foto\">foto</a>";

				$vypis = $vypis." | <a href=\"".$adresa_modulu."?id_zapas=".$this->radek["id_zapas"]."&amp;typ=zapas&amp;pozadavek=delete\" onclick=\"javascript:return confirm('Opravdu chcete smazat objekt?')\">delete</a>";
				
				$vypis = $vypis." <form style=\"font-size:0.9em;\" action=\"".$adresa_modulu."?id_zapas=".$this->radek["id_zapas"]."&amp;typ=zapas&amp;pozadavek=copy\" method=\"post\"><strong>kopírovat zápas</strong>: nový název <input type=\"text\" name=\"nazev\" /><input type=\"submit\" value=\"&gt;&gt;\" /></form>";
