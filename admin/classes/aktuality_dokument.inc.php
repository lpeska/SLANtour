...
class Dokument_aktuality extends Generic_list{
	...
	//------------------- KONSTRUKTOR -----------------
	/**konstruktor tøídy*/
	function __construct($typ_pozadavku,$id_zamestnance,$id_aktuality,$id_dokument=""){
	...
	//kontrola dat
		...
		if($this->legal($this->typ_pozadavku,$this->id_zamestnance,$this->id_aktuality)){
...
//------------------- METODY TRIDY -----------------	
	...
	function create_query($typ_pozadavku){
		if($typ_pozadavku=="show"){
			$dotaz="select `dokument_aktuality`.`id_aktuality`, `dokument`.`id_dokument`,`dokument`.`dokument_url`,
							`dokument`.`nazev_dokument`,`dokument`.`popisek_dokument`  
					  from `dokument_aktuality` join
							`dokument` on (`dokument`.`id_dokument` =`dokument_aktuality`.`id_dokument`) 
					where `dokument_aktuality`.`id_aktuality`= ".$this->id_aktuality." 
			...
			$dotaz=$dotaz.
						"INSERT INTO `dokument_aktuality`
							(`id_aktuality`,`id_dokument`)
						VALUES
							(".$this->id_aktuality.",".$this->id_dokument.");";
			//echo $dotaz;
			return $dotaz;
		
		}else if($typ_pozadavku=="delete"){
			$dotaz= "DELETE FROM `dokument_aktuality` 
						WHERE `id_aktuality`=".$this->id_aktuality." and `id_dokument`=".$this->id_dokument."
						LIMIT 1";
			//echo $dotaz;
			return $dotaz;		
	...
	function show_list_item($typ_zobrazeni){
		if($typ_zobrazeni=="tabulka"){
			...
			$vypis=$vypis."
							...
							<td class=\"menu\">
							  <a href=\"?id_aktuality=".$this->get_id_aktuality()."&amp;id_dokument=".$this->get_id_dokument()."&amp;typ=dokument&amp;pozadavek=delete\">odebrat</a>
							</td>
						</tr>";
			return $vypis;
		}
	...
	function get_id_aktuality() { return $this->radek["id_aktuality"];}
