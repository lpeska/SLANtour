 ...
 */
function createCartData($goodsDesc, $totalAmount) {
...
function createGetParamsEcho($merchantId, $dttm, $privateKey, $privateKeyPassword) {
    $text = $merchantId . "|" . $dttm;
    $signature = sign($text, $privateKey, $privateKeyPassword, "data to sign:");
    return $merchantId . "/" . $dttm . "/" . urlencode($signature);
}
...
function preparePutRequestEcho($merchantId, $dttm, $privateKey, $privateKeyPassword) {
    $data = array(
        "merchantId" => $merchantId,
        "dttm" => $dttm
    );
